...
export function getDefaultStealthArgs(): string[] {
  ...
  // --no-sandbox policy:
  // - Never on native Windows/macOS (real Chrome doesn't; Google returns 400)
  // - Linux containers only (via shouldDisableSandbox heuristics)
  // - Force on any platform with CLOAKBROWSER_NO_SANDBOX=1
  if (shouldDisableSandbox()) {
    base.push("--no-sandbox");
  }
...
function shouldDisableSandbox(): boolean {
  // Explicit force via env var (escape hatch, works on all platforms)
  if (process.env.CLOAKBROWSER_NO_SANDBOX === "1") {
    return true;
  }

  // Container heuristics only make sense on Linux
  ...
  // Running as root (very common in containers)
  try {
    if (process.getuid && process.getuid() === 0) return true;
  } catch {}

  // Other container hints
  const hints = ["KUBERNETES_SERVICE_HOST", "container", "DOCKER_CONTAINER"];
  for (const h of hints) {
    if (process.env[h]) return true;
  }
