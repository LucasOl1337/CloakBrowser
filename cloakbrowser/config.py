...
def get_default_stealth_args() -> list[str]:
    ...
    --no-sandbox is ONLY added on Linux when running in a containerized
    environment (Docker, running as root, etc.). On native Windows and macOS
    it is never used — real Chrome does not run with --no-sandbox there,
    and Google explicitly rejects it on sign-in flows (causing 400 errors).
    """
    ...
    # --no-sandbox policy:
    # - Never on native Windows/macOS (real Chrome doesn't use it; Google 400s)
    # - On Linux only inside containers (detected via _should_disable_sandbox)
    # - Can be forced on any platform with CLOAKBROWSER_NO_SANDBOX=1
...
def _should_disable_sandbox() -> bool:
    """Return True if we should pass --no-sandbox.

    Real desktop Chrome (Windows/macOS/Linux) never runs with --no-sandbox.
    Google treats it as a security risk and returns 400 on sign-in flows.

    We normally only enable it for Linux containers. The env var
    CLOAKBROWSER_NO_SANDBOX=1 can be used to force it on any platform
    (advanced users / special container setups).
    """
    # Explicit force (works on all platforms as an escape hatch)
    if os.environ.get("CLOAKBROWSER_NO_SANDBOX") == "1":
        return True

    # Only consider container heuristics on Linux
    if platform.system() != "Linux":
        return False

    # Classic Docker indicator
    if os.path.exists("/.dockerenv"):
        return True

    # Running as root inside a container is a very strong signal
    try:
    ...
    # Additional common container env vars
    container_hints = (
